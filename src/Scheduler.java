...
public abstract class Scheduler {
    ...
    private int[] state_counter = { 0, 0, 0, 0, 0 }; // {new,ready,running,waiting,terminated} contains process number
                                                     // of each state
    ...
    private String CPU, IO, ave_tur_t, ave_wai_t, Throughput_processes; // Percentage of time some job is
                                                                        // running/blocked , average turnaround/waiting
                                                                        // time , Throughput_processes
    ...
    public int running_check(int[] s_c, pcb[] p, Queue<pcb> q, int r) {
        return scheduling.running_check(s_c, p, q, r);
    }

    public void waiting_check(int[] s_c, pcb[] p, Queue<pcb> q) {
        scheduling.waiting_check(s_c, p, q);
    }
...
class FCFSorSJF implements scheduling {
    public int running_check(int[] s_c, pcb[] p, Queue<pcb> q, int r) {
        if (p[r].isRunning()) {
            ...
            } else if (p[r].getRandomizedCPUburst() == 0 && p[r].getRemainingCPUT() != 0) { // set state into terminated
                                                                                            // when cpu burst is 0
    ...
    public static void waitingCheck(int[] s_c, pcb[] p, Queue<pcb> q) {
        for (int i = 0; i < p.length; i++) {
            if (p[i].isWaiting()) { // check every process whether is waiting
                if (p[i].getRandomizedIOburst() == 0) { // if io burst is 0, set process state into ready and add to
                                                        // ready_queue also give random cpu burst and io burst
...
class RR implements scheduling {
    public int running_check(int[] s_c, pcb[] p, Queue<pcb> q, int r) {
        if (p[r].isRunning()) {
            if (p[r].getRemainingCPUT() == 0) {
                ...
                s_c[4]++;
                return popAndRun(s_c, p, q);
            } else if (p[r].getRemainingQuantum() == 0 && p[r].getRandomizedCPUburst() > 0
                    && p[r].getRemainingCPUT() != 0) { // if quantum is over, set process state into ready and add to
                                                       // ready_queue
    ...
    public void waiting_check(int[] s_c, pcb[] p, Queue<pcb> q) {
        FCFSorSJF.waitingCheck(s_c, p, q);
    }
