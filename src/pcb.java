...
public class pcb implements Comparable<pcb> {
    private int pid;
    private int arrivalT, totalCPUT, CPUburst, IOburst; // arrival time, total CPU time, Maximum CPU burst time, Maximum
                                                        // IO burst time
    private int quantum, RemainingQuantum; // Using in round-robin
    private int remainingCPUT, randomizedCPUburst, randomizedIOburst; // remaining cpu time, randomized cpu burst ,
                                                                      // randomized io burst
    // Finishing time , Turnaround time, CPU time (time that remained on running
    // state)
    // IO time (time that remained on blocked state), Waiting time (time that
    // remained on ready state)
    ...
    public void printInfo() {
        ...
        System.out.println("Arrival time : " + this.arrivalT + ", Total CPU time : " + this.totalCPUT
                + ", CPU burst bound : " + this.CPUburst + ", IO burst bound : " + this.IOburst);
    ...
    public int compareTo(pcb p) { // using when enqueuing to priority queue
        if (this.randomizedCPUburst < p.getRandomizedCPUburst())
            return -1;
        else if (this.randomizedCPUburst > p.getRandomizedCPUburst())
            return 1;
        return 0;
    }
}
